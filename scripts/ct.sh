...
#
# Usage:  scripts/ct.sh
#         scripts/ct.sh --build-only        # compile ct_main only; never run
#                                              it (RFC-005 slice 16, below)
#         SELLO_IN_CONTAINER=1 scripts/ct.sh [...]   # already inside the
#                                                       pinned image
#
# --build-only (RFC-005 slice 16, the build-smoke check): compiles
# tests/ct/ct_main.nim (-d:release, identical flags to the normal path)
# and stops -- the binary is never invoked, so this produces no timing
# samples and no verdict of any kind. This is deliberately the ONLY thing
# hosted CI ever does with this harness (RFC-005 Part B: "hosted CI runs
# the dudect harness in compile-smoke mode only -- no verdict authority");
# the real >= 1e6-samples/class battery this script normally runs is
# environment-sensitive (shared-host noise, CPU governor, see the
# preflight banner below) in a way no merge-gate-required check can
# tolerate, and stays a maintainer-run, manually-interpreted instrument
# (docs/ct-results.md), never a green/red CI signal. The environment
# preflight banner below (CPU governor, running containers, load average)
# is skipped in this mode -- it exists to caveat a REAL timing run, and a
# compile-only invocation produces no timing data for it to caveat.
#
...
# podman wrapper and no host milpa-lock preflight, same rationale as
# every other dual-mode script in this project (no host to preflight-
# check from inside a container CI already runs in; nesting podman
# inside a CI container job has no podman binary to nest with anyway).
#
...
if [[ "$build_only" -eq 0 ]]; then
  ...
  # environment, not intolerance of a noisy one). Skipped entirely under
  # --build-only (see that flag's own header paragraph above): it exists
  # to caveat a REAL timing run, and a compile-only invocation produces no
  # timing data for it to caveat.
  echo "=============================================================="
...
echo "ct.sh --build-only: ct_main compiled -- COMPILE-SMOKE ONLY, not run."
echo "ct.sh --build-only: no timing samples were collected and no verdict of any kind was produced."
echo "ct.sh --build-only: the real >= 1e6-samples/class dudect battery runs only via a plain, maintainer-invoked scripts/ct.sh; see docs/ct-results.md."
'
else
  cmd='set -e
nim c -d:release --outdir:build tests/ct/ct_main.nim
