...
- **Resume:** implement the next unchecked slice below in a sonnet subagent (RED-GREEN-
  REFACTOR, full container suite green, per-slice commit after gates — Corey endorsed the
  RFC-001 per-batch commit pattern). Build/test: `scripts/test.sh`, `scripts/test-libsodium.sh`
  (sello-dev image), `scripts/check-readme.sh`, `scripts/fuzz.sh`, `scripts/bmc.sh`,
  `scripts/ct.sh`; single file via the podman + milpa-CAS-mount invocation in CLAUDE.md.
  `rm` is aliased interactive — use `rm -f`. proptest repo is read-only reference.

## Slices
- [ ] 1 API coherence — actor-first `pk.verify(msg, sig)`; `toBytes(kp)` + delete `seed()`;
