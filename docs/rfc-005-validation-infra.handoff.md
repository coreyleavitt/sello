...
- **`docs/rfc-005-validation-infra.closeout-audit.md` (new, deliverable (a)).** One row
...
**Slice 32 audit-fix follow-up (this session, branch `rfc-005-slice32-audit-fixes` from `main` HEAD `8914923`):** both close-out-audit findings closed. Finding 1: README's `coverage-ratchet` row Mechanism cell reworded to "single-pass by default (`--verify-determinism` opts into a build+run-twice determinism check)", matching CLAUDE.md's own already-accurate wording. Finding 2: CLAUDE.md's "The validation bar" bullet list gained a disassembly-gate bullet (after the taint-based CT harness bullet, citing red-demo run `33309730168`), and README's validation-map table gained the `disasm-gate` row (Category `required-check`, Mechanism `disasm-gate-gcc` (and `disasm-gate-clang`), Carve-out doc `tests/ct_disasm/expected/justifications.md`). `docs/rfc-005-validation-infra.closeout-audit.md`'s two finding entries now each carry a "Fixed in commit" line citing `7aa37a0` (the fix) and `4989c75` (the doc's own follow-up commit recording that hash). `scripts/validation-map-check.sh` (`python3 scripts/lib/validation_map_check.py`) and `scripts/check-readme.sh` both run clean locally. Merge-gate run `33325962020` on `main` HEAD `4989c75`, 27/27 required checks green; fast-forwarded to `main` (`8914923..4989c75`), branch deleted both remote and local (confirmed 404).
