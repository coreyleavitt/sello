...
- **Stage:** 3 (implement) — slice grind in progress; 3/11 done
...
- [x] 3 compile-time base table (GeCached z=1) + CT full-scan select (identity-initialized output) — full suite green (hash backfilled at slice 4, per the slice-2 precedent above: self-referencing a commit's own hash from inside itself is circular); GeBaseTable[i][j] = cached((j+1)*256^i * B), built via 248 compile-time doublings + chained additions (~1s, no const-eval blocker); standing-guard test compares against vartime scalarmult via canonical point encoding (pointEncode), not raw GeCached struct equality — two different algorithms reaching the same point generally land on different, equally-valid projective (X:Y:Z:T) representatives, so raw limb comparison is a false-negative trap; cmovCached covers digit 0 (identity) and all of -8..8 with sign handling, across every table row
